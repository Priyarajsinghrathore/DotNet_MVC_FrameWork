$(document).ready(function () {
    ...
    $(document).on('click', '.EditRecordCLS', function () {
      ...
      $("#DDL_PageCounter").change(function () {
        ...
        $(document).on('click', '.EditRecordStatusCLS', function () {
                ...
                if (WhereClause == 'Where Id=') {
                    ...
                    for (var i = 0; i < UserArr.length; i++) {
                        ...
                        $("#UserTableBody").append(
                            ...
                            + "<td class='text-center'><a class='EditStatusCLS' data-UserIdStatus=" + UserArr[i].Id + " data-UserStatus=" + UserArr[i].Status + ">" + StatusIcon + "</a></td>"
                            + "</tr>"
                        )

                        $("#UserTableBody tr").css("display", "none");
                        $("[data-row-id=1]").css("display", "table-row"); var MaxPagerCounter = parseInt(UserArr.length / parseInt($("#DDL_PageCounter").val()));
                        ...
                        if (MaxPagerCounter <= 1 && (MaxPagerCounterMod == 0 || parseInt(UserArr.length) < parseInt($("#DDL_PageCounter").val()))) {
                            ...
                            $("#ManagePager").empty();
                            $("#ManagePager").append("<li class='page-item' data-indexid='-1'><a class='page-link'><i class='fa fa-angle-double-left'></i> Previous</a></li><li class='page-item active' data-indexid='1'><a class='page-link'>1</a></li>"); if (MaxPagerCounterMod > 0) {
                                MaxPagerCounter += 1;
                            }
...
function UpdateRecord() {
    $.ajax({
        ...
        data: '{Id:' + parseInt($("#BtnUpdateRecord").attr('EditUserId')) + ',FirstName: "' + $("#TxtFName").val().trim() + '",LastName: "' + $("#TxtLName").val().trim() + '",ContactNo: "' + $("#TxtContactNo").val().trim() + '",EmailId: "' + $("#TxtEmailId").val().trim() + '",Password: "",Gender: "' + $("#DDL_Gender").val() + '" }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
